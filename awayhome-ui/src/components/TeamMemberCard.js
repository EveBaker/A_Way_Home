...
import { Card, CardHeader, CardBody, CardFooter, Typography, Avatar } from '../app/MTailwind';

const TeamMemberCard = ({ name, title, avatar, socialLinks }) => {
    const getLinkText = (type, url) => {
        switch (type) {
            case 'github':
    ...
    return (
        <Card className="p-4 rounded-lg shadow-md text-center border-3 border-primary-blue">
            <CardHeader className="flex justify-center bg-primary-blue">
                <Avatar src={avatar} alt={name} size="xxl" className="mb-4 border-2 border-bright-teal" />
            </CardHeader>
            <CardBody>
            ...
            <CardFooter>
                {socialLinks.map((link) => (
                    <Typography variant="body2" key={link.type}>
                        <a href={link.type === 'email' ? `mailto:${link.url}` : link.type === 'phone' ? `tel:${link.url}` : link.url}
